...
const Restaurant_Editor = ({Restaurant, Set_Restaurant, Set_Users, Users}) =>
{
	...
	const Change_the_Restaurant_Icon = async New_Icon =>
	{
		...
		Restaurant_Copy.Icons.Five_Hundred_Twelve_Pixels_File_Path = `/Images/${Restaurant_Copy.Name.replace (' ', '_')}/${Restaurant_Copy.Name.replace (' ', '_')}_Icon_512_Pixels${Get_the_File_Extension (New_Icon.type)}`
	...
	const Change_the_Restaurant_Logo = async New_Logo =>
	{
		...
		Restaurant_Copy.Logo_File_Path = `/Images/${Restaurant_Copy.Name.replace (' ', '_')}/${Restaurant_Copy.Name.replace (' ', '_')}_Logo${Get_the_File_Extension (New_Logo.type)}`
	...
	return (
		<div className='Restaurant_Editor' key='Restaurant_Editor_Key'>
			...
			<div className='Managers'>
				{
					Users.filter (User => User.Type !== 'Owner').map (User => Manager_Editing_Statuses [User.Username] ? <Manager_Editor Delete_the_User={() => Delete_the_User (User.Username)} key={`${User.Username}_Edtior`} Save_the_User={() => Save_the_User (User.Username)} User={User}></Manager_Editor> : <Manager_Card Delete_the_User={() => Delete_the_User (User.Username)} Edit_the_User={() => Edit_the_User (User.Username)} Email={User.Email} key={`${User.Username}_Card`} Username={User.Username}></Manager_Card>)
				}
				{
					New_User_Creation_Mode ? <Manager_Editor Delete_the_User={() => Set_New_User_Creation_Mode (false)} key='New_Manager_Edtior' User={{New_Status: true}} Users={Users} Set_Users={Set_Users} Set_New_User_Creation_Mode={Set_New_User_Creation_Mode}></Manager_Editor> : <Addition_Button Color='#8068A8' Function={() => Set_New_User_Creation_Mode (true)}></Addition_Button>
				}
			</div>
		</div>
	)
